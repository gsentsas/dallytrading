      ...
      data: {
        order: {
          ...
          delivery: {
            ...
            fee: { status: 'free', amount: 0, currency: 'XOF' },
...
describe('commande invité', () => {
  ...
  it('transmet une adresse de livraison validée, jamais un frais', async () => {
    fetchMock.mockResolvedValueOnce(commandeOdoo());
    ...
    const envoye = JSON.parse(init.body as string);
    expect(envoye.deliveryMode).toBe('dakar-express');
    expect(envoye.shipping).toEqual({
      name: 'Destinataire', street: 'Rue 10', city: 'Dakar', country_code: 'SN',
    });
    expect(JSON.stringify(envoye)).not.toContain('deliveryFee');
  });
...
describe('panier', () => {
  it('refuse un cookie altéré sans créer de commande', async () => {
    ...
    const response = await (await POST())(requete({ cartCookie: morceaux.join('.') }));
  ...
  it('refuse un panier portant un prix injecté', async () => {
    const contrebande = {
      ...newCart(), lines: [{ reference: 'filtre-a-huile', quantity: 1, price: 1 }],
    };
    const response = await (await POST())(
...
describe('sécurité de la mutation', () => {
  ...
  it('refuse un frais injecté par le navigateur avant Odoo', async () => {
    const { cookie } = panierScelle();
    const response = await (await POST())(
      requete({
        cartCookie: cookie,
        body: { deliveryMode: 'pickup', customer: INVITE, deliveryFee: 1 },
      }),
    );
    expect(response.status).toBe(422);
    expect(fetchMock).not.toHaveBeenCalled();
