...
export default async function CommandePage({
  ...
  return (
    <>
      ...
      <Link href="/espace-client/commandes" className="mb-6 inline-flex text-sm text-navy-800 underline">
      ...
      <Card>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <dl className="grid flex-1 gap-4 sm:grid-cols-2">
            ...
            <Detail label="Remise" value={commande.fulfillmentLabel} />
          </dl>
        ...
        {commande.state === 'received' && (
          <p className="mt-6 rounded-lg border border-mist-200 bg-mist-50 p-4 text-sm text-mist-700">
            Nous avons bien reçu cette commande. Nos équipes vérifient la disponibilité, les frais de remise et les modalités de règlement. Aucun paiement n’a été demandé.
          </p>
        )}
        {commande.state === 'validated' && !['delivered', 'picked_up'].includes(commande.fulfillmentState) && (
          <p className="mt-6 rounded-lg border border-green-200 bg-green-50 p-4 text-sm text-green-900">
            Votre commande est validée commercialement. Son état de remise est affiché ci-dessus ; la préparation n’est engagée qu’après autorisation explicite de nos équipes.
          </p>
        )}
        {commande.state === 'rejected' && (
          <p className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            Cette commande ne peut pas être validée dans son état actuel. Le motif communiqué figure dans le statut ci-dessus.
          </p>
        )}
        {commande.state === 'cancelled' && (
          <p className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            Cette commande a été annulée. Le motif communiqué figure dans le statut ci-dessus.
          </p>
        )}
      </Card>
      ...
      <TableWrapper>
        <table className="mt-3 w-full min-w-[36rem] border-collapse bg-white text-left">
          <caption className="sr-only">Articles de la commande {commande.reference}</caption>
          <thead>
          ...
          <tbody>
            {commande.lines.map((ligne, index) => (
              <tr key={`${ligne.productName}-${index}`} className="border-b border-mist-200">
                <td className="p-3 text-navy-800">{ligne.productName}</td>
                <td className="p-3 text-mist-700">{formatQuantity(ligne.quantity)}</td>
                <td className="p-3 text-mist-700">{formatOrderAmount(ligne.unitPrice, commande.currency)}</td>
                <td className="p-3 font-medium text-navy-800">{formatOrderAmount(ligne.subtotal, commande.currency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card className="mt-8">
        <dl className="grid gap-3 sm:max-w-sm sm:ml-auto">
          <div className="flex justify-between text-mist-700">
            <dt>Sous-total</dt><dd>{formatOrderAmount(commande.amountUntaxed, commande.currency)}</dd>
          </div>
          <div className="flex justify-between text-mist-700">
            <dt>Taxes</dt><dd>{formatOrderAmount(commande.amountTax, commande.currency)}</dd>
          </div>
          <div className="flex justify-between text-mist-700">
            <dt>Frais de remise</dt>
            <dd>{commande.deliveryFee === null ? 'À confirmer' : formatOrderAmount(commande.deliveryFee, commande.currency)}</dd>
          </div>
          <div className="flex justify-between border-t border-mist-200 pt-3 text-lg font-semibold text-navy-900">
            <dt>{commande.grandTotal === null ? 'Total articles' : 'Total'}</dt>
            <dd>{formatOrderAmount(commande.grandTotal ?? commande.amountTotal, commande.currency)}</dd>
          </div>
        </dl>
        {commande.deliveryFeeStatus === 'pending_quote' && (
          <p className="mt-4 text-xs text-mist-500 sm:text-right">
            Les frais de remise seront confirmés par DallyTrading avant l’autorisation de préparation.
          </p>
        )}
      </Card>
      ...
      {commande.deliveryAddress && (
        <>
          <h2 className="mt-8 text-lg font-semibold text-navy-900">Adresse de livraison</h2>
          <Card className="mt-3">
